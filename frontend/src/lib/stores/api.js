...
function createAuthStore() {
  ...
  return {
    ...
    async register(username, email, password, avatar = '👤') {
      const data = await apiFetch('/api/auth/register', { method:'POST', body: JSON.stringify({username,email,password,avatar}) });
    ...
    async oauth(provider, provider_id, email, username, avatar) {
      const data = await apiFetch('/api/auth/oauth', { method:'POST', body: JSON.stringify({provider,provider_id,email,username,avatar}) });
      persist(data.user, data.token);
      set({ user: data.user, token: data.token });
      return data.user;
    },
...
export async function apiFetch(path, opts = {}) {
  ...
  const res = await fetch(`${API_URL}${path}`, { headers, ...fetchOpts });
